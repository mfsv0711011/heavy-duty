...
import {useAdditionalExpenseStore} from "@/stores/modules/additionalExpense.js";
import useMaska from "@/composable/useMaska.js";

const materialStore = useMaterialStore()
const materialInvoiceStore = useMaterialInvoiceStore()
const additionalExpenseStore = useAdditionalExpenseStore()
...
const editMaterialInvoice = handleSubmit(materialInvoice => {
    const invoice = materialInvoiceStore.getMaterialInvoice

    const materialInvoiceData = {
        id: invoice.id
    }

    if (material.value !== invoice.material['@id']) {
        materialInvoiceData.material = materialInvoice.material
    }

    if (provider.value !== invoice.provider) {
        materialInvoiceData.provider = materialInvoice.provider
    }

    if (+qty.value.replaceAll(' ', '') !== invoice.qty) {
        materialInvoiceData.qty = +materialInvoice.qty.replaceAll(' ', '')
    }

    if (hasNds.value !== invoice.hasNds) {
        materialInvoiceData.hasNds = materialInvoice.hasNds
    }

    if (+price.value.replaceAll(' ', '') !== invoice.price) {
        materialInvoiceData.price = +materialInvoice.price.replaceAll(' ', '')
    }

    if (currency.value !== invoice.currency['@id']) {
        materialInvoiceData.currency = materialInvoice.currency
    }

    if (usdRate.value !== invoice.usdRate) {
        materialInvoiceData.usdRate = +materialInvoice.usdRate
    }

    let changedAdditionalExpense = false

    for (let i = 0; i < additionalExpensesList.value.length; i++) {
        if (
            additionalExpensesList.value[i].name !== invoice.additionalExpenses[i].name ||
            additionalExpensesList.value[i].currency !== invoice.additionalExpenses[i].currency['@id'] ||
            additionalExpensesList.value[i].sum !== invoice.additionalExpenses[i].sum
        ) {
            additionalExpenseStore.putAdditionalExpense(additionalExpensesList.value[i])
            changedAdditionalExpense = true
        }
    }

    if (date.value !== formatDate(invoice.createdAt) || time.value !== formatTime(invoice.createdAt)) {
        materialInvoiceData.createdAt = `${date.value}T${time.value}`
    }

    if (Object.keys(materialInvoiceData).length > 1 || changedAdditionalExpense) {
        isLoading.value = true
        materialInvoiceStore.putMaterialInvoice(materialInvoiceData)
            .then(() => {
...
onMounted(async () => {
    ...
    material.value = materialInvoiceStore.getMaterialInvoice.material['@id']
    provider.value = materialInvoiceStore.getMaterialInvoice.provider
    qty.value = formatCurrency(materialInvoiceStore.getMaterialInvoice.qty)
    hasNds.value = materialInvoiceStore.getMaterialInvoice.hasNds
    usdRate.value = materialInvoiceStore.getMaterialInvoice.usdRate
    price.value = formatCurrency(materialInvoiceStore.getMaterialInvoice.price)
    currency.value = materialInvoiceStore.getMaterialInvoice.currency['@id']
    additionalExpensesList.value = materialInvoiceStore.getMaterialInvoice.additionalExpenses.map(addExp => {
        return {
            id: addExp.id,
            name: addExp.name,
            currency: addExp.currency['@id'],
            sum: +addExp.sum
        }
    })
    date.value = formatDate(materialInvoiceStore.getMaterialInvoice.createdAt)
    time.value = formatTime(materialInvoiceStore.getMaterialInvoice.createdAt)
